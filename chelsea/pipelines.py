...
class ChelseaPipeline:
    ...
    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `chelsea`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM chelsea WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO chelsea (date, title, link, content)"
